...
const OutputNodeComponent = ({ id, selected, ...node }: NodeProps) => {
  ...
  return (
    <div className={LANGCHAIN_NODE_OUTER_CLASS}>
      ...
      <div
        className={`border-2 border-green-400 bg-green-50 dark:bg-green-950 ${LANGCHAIN_NODE_INNER_CLASS}`}
      >
        ...
        <p className={`text-green-600/90 dark:text-green-400/90 ${LANGCHAIN_NODE_LABEL_CLASS}`}>
          {data.label}
        </p>
        <div
          className={`text-green-600 bg-green-100 dark:bg-green-900 rounded px-2 py-0.5 ${LANGCHAIN_NODE_PREVIEW_CLASS}`}
        >
          {preview}
        </div>
      </div>
