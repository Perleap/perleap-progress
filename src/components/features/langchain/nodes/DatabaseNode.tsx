...
const DatabaseNodeComponent = ({ id, selected, ...node }: NodeProps) => {
  ...
  return (
    <div className={LANGCHAIN_NODE_OUTER_CLASS}>
      ...
      <div
        className={`border-2 border-indigo-400 bg-indigo-50 dark:bg-indigo-950 ${LANGCHAIN_NODE_INNER_CLASS}`}
      >
        ...
        <p className={`text-indigo-600/90 dark:text-indigo-400/90 ${LANGCHAIN_NODE_LABEL_CLASS}`}>
          {data.label}
        </p>
        <div
          className={`text-indigo-600 bg-indigo-100 dark:bg-indigo-900 rounded px-2 py-0.5 ${LANGCHAIN_NODE_PREVIEW_CLASS}`}
        >
          {preview}
        </div>
      </div>
