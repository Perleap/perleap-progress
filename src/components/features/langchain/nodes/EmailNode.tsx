...
const EmailNodeComponent = ({ id, selected, ...node }: NodeProps) => {
  ...
  return (
    <div className={LANGCHAIN_NODE_OUTER_CLASS}>
      ...
      <div
        className={`border-2 border-teal-400 bg-teal-50 dark:bg-teal-950 ${LANGCHAIN_NODE_INNER_CLASS}`}
      >
        ...
        <p className={`text-teal-600/90 dark:text-teal-400/90 ${LANGCHAIN_NODE_LABEL_CLASS}`}>
          {data.label}
        </p>
        <div
          className={`text-teal-600 bg-teal-100 dark:bg-teal-900 rounded px-2 py-0.5 ${LANGCHAIN_NODE_PREVIEW_CLASS}`}
        >
          {preview}
        </div>
      </div>
