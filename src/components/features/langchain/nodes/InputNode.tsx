...
const InputNodeComponent = ({ id, selected, ...node }: NodeProps) => {
  ...
  return (
    <div className={LANGCHAIN_NODE_OUTER_CLASS}>
      ...
      <div
        className={`border-2 border-blue-400 bg-blue-50 dark:bg-blue-950 ${LANGCHAIN_NODE_INNER_CLASS}`}
      >
        ...
        <p className={`text-blue-600/90 dark:text-blue-400/90 ${LANGCHAIN_NODE_LABEL_CLASS}`}>
          {data.label}
        </p>
        <div
          className={`text-blue-600 bg-blue-100 dark:bg-blue-900 rounded px-2 py-0.5 ${LANGCHAIN_NODE_PREVIEW_CLASS}`}
        >
          {preview}
        </div>
      </div>
