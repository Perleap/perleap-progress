...
import { ensureLangchainNodeData, type LangchainTriggerNodeData } from '../langchainNodeData';
...
const TriggerNodeComponent = ({ id, selected, ...node }: NodeProps) => {
  ...
  return (
    <div className={LANGCHAIN_NODE_OUTER_CLASS}>
      ...
      <div
        className={`border-2 border-orange-400 bg-orange-50 dark:bg-orange-950 ${LANGCHAIN_NODE_INNER_CLASS}`}
      >
        ...
        <p className={`text-orange-600/90 dark:text-orange-400/90 ${LANGCHAIN_NODE_LABEL_CLASS}`}>
          {data.label}
        </p>
        <div
          className={`text-orange-600 bg-orange-100 dark:bg-orange-900 rounded px-2 py-0.5 ${LANGCHAIN_NODE_PREVIEW_CLASS}`}
        >
          {modeLabel}
        </div>
      </div>
