...
import { cleanTextForTTS } from './chatTtsUtils';
import {
  CHAT_INPUT_MAX_HEIGHT_PX,
  COMPANION_CHAT_OPEN_LS,
  type AssignmentChatInterfaceProps,
} from './types';
...
import {
  ...
  reportAssignmentChatSentence,
  uploadChatAttachmentToStorage,
} from '@/services/assignmentChatService';
...
export const AssignmentChatInterface = ({
  ...
  const dbAdminQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return checkIsAppAdmin(user.id);
    },
  });
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (isText) {
          ...
          const { filePath, isImage, safeName } = await uploadChatAttachmentToStorage(
            submissionId,
            file
          );
          setAttachedFile({
  ...
  const handleSubmitSentenceFlags = async () => {
    ...
    try {
      ...
      const dbMessages = await fetchConversationMessages(submissionId);
      ...
      for (const si of sortedIdx) {
        ...
        const r = await reportAssignmentChatSentence({
          submissionId,
