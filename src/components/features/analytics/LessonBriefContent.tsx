...
export const LessonBriefContent = () => {
  ...
  const submissionScopeSet = useMemo(
    () => new Set(effectiveAssignmentIds),
  ...
  const classPriorityInsights = useMemo(() => {
    ...
    const firstWeakScore =
      firstWeakDimension && classAverage ? safeScore(classAverage[firstWeakDimension]) : 0;
  ...
  useEffect(() => {
    ...
    const buildBaseStudents = () => {
      ...
      return list.map((st) => ({
        ...
        scores: scopedStudentLatestScores(
          st.snapshots,
          data.rawSubmissions,
          effectiveAssignmentIds
        ),

        qedMeasures: scopedStudentLatestQedMeasures(
          st.snapshots,
          data.rawSubmissions,
          effectiveAssignmentIds
        ),
    ...
    const waitForPreload = (): Promise<boolean> =>
      new Promise((resolve) => {
        ...
        pollTimer = setInterval(() => {
          ...
          const status = getLessonBriefPreloadStatus(
            classroomId,
            selectedModule,
            selectedAssignment
          );
