...
import type {
  ...
  SyllabusPolicyType,
  ReleaseMode as ReleaseModeType,
} from '@/types/syllabus';
      ...
      <div className="space-y-6">
        ...
        <Card className="rounded-xl border-destructive/30 bg-destructive/5">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm mb-4">{t('syllabus.loadFailedDesc')}</p>
        ...
        <Card className="rounded-xl border-dashed border-2 border-border bg-muted/20">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-xl font-bold text-foreground mb-2">
              {t('syllabus.noSyllabusYet')}
            </h3>
            <p className="text-muted-foreground max-w-md mb-6">{t('syllabus.noSyllabusDesc')}</p>
          ...
          <CardContent className="p-6 space-y-5">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={t('syllabus.titlePlaceholder')}
        ...
        {syllabus.status === 'draft' && (
          <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <Button
              ...
              disabled={publishMutation.isPending || syllabus.sections.length === 0}
...
const SETTINGS_PRESETS: {
  ...
  icon: typeof GradIcon;
  colorClass: string;
}[] = [
  {
    type: 'grading',
    labelKey: 'syllabus.policies.grading',
    fallbackLabel: 'Grading Policy',
    icon: GradIcon,
    colorClass: 'text-blue-500 bg-blue-100 dark:bg-blue-900/30',
  },
  {
    type: 'attendance',
    labelKey: 'syllabus.policies.attendance',
    fallbackLabel: 'Attendance Policy',
    icon: AttIcon,
    colorClass: 'text-green-500 bg-green-100 dark:bg-green-900/30',
  },
  {
    type: 'late_work',
    labelKey: 'syllabus.policies.lateWork',
    fallbackLabel: 'Late Work Policy',
    icon: LateIcon,
    colorClass: 'text-orange-500 bg-orange-100 dark:bg-orange-900/30',
  },
  {
    type: 'communication',
    labelKey: 'syllabus.policies.communication',
    fallbackLabel: 'Communication Policy',
    icon: CommIcon,
    colorClass: 'text-purple-500 bg-purple-100 dark:bg-purple-900/30',
  },
  {
    type: 'academic_integrity',
    labelKey: 'syllabus.policies.academicIntegrity',
    fallbackLabel: 'Academic Integrity',
    icon: IntegrityIcon,
    colorClass: 'text-red-500 bg-red-100 dark:bg-red-900/30',
  },
  {
    type: 'participation',
    labelKey: 'syllabus.policies.participation',
    fallbackLabel: 'Participation',
    icon: ParticIcon,
    colorClass: 'text-teal-500 bg-teal-100 dark:bg-teal-900/30',
  },
  {
    type: 'extra_credit',
    labelKey: 'syllabus.policies.extraCredit',
    fallbackLabel: 'Extra Credit',
    icon: CreditIcon,
    colorClass: 'text-yellow-500 bg-yellow-100 dark:bg-yellow-900/30',
  },
];

const SyllabusSettings = ({
  syllabus,
  editingMeta,
  onStartEdit,
  onSave,
          ...
          <Card className="rounded-xl border-border shadow-sm">
            <CardContent className="p-5 space-y-4">
              ...
              <SettingsField
                ...
                value={syllabus.structure_type}
                isRTL={isRTL}
              />
              <div className={cn('grid gap-4 sm:grid-cols-2', isRTL ? 'text-right' : 'text-left')}>
                <SettingsField
                  label={t('createClassroom.startDate')}
                  value={formatCourseDateLabel(courseStartDate)}
                  isRTL={isRTL}
                />
                <SettingsField
                  label={t('createClassroom.endDate')}
                  value={formatCourseDateLabel(courseEndDate)}
                  isRTL={isRTL}
                />
              </div>
              <SettingsField
                ...
                value={t(
                  `syllabus.releaseMode.${syllabus.release_mode || 'all_at_once'}`,
            ...
            <CardContent className="p-5 space-y-4">
              {policies.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {t('syllabus.policies.noPolicies', 'No policies defined.')}
                </p>
              ) : (
                policies.map((p) => (
                  <SettingsField
  ...
  const removePolicy = (id: string) => {
    onChangePolicies(
      metaPolicies.filter((p) => p.id !== id).map((p, i) => ({ ...p, order_index: i }))
    );
  };
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['weeks', 'units', 'modules'] as const).map((sType) => (
                  <button
                    key={sType}
                    type="button"
                    onClick={() => onChangeStructureType(sType)}
                    className={cn(
                      'p-2 rounded-lg border-2 text-center text-sm transition-all',
                      metaStructureType === sType
                ...
                <PopoverContent align="end" className="w-64 p-2 rounded-xl" sideOffset={8}>
                  <div className="space-y-0.5">
                    {SETTINGS_PRESETS.map((preset) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            addPolicy(preset.type, t(preset.labelKey, preset.fallbackLabel))
                          }
                          className={cn(
                        ...
                        >
                          <div className={cn('p-1 rounded-md', preset.colorClass)}>
                            <PIco className="h-3 w-3" />
                          </div>
                    ...
                    <button
                      ...
                      className={cn(
                        'w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm hover:bg-muted/60',
                        isRTL && 'flex-row-reverse text-right'
                      )}
                    >
                      <div className="p-1 rounded-md text-gray-500 bg-gray-100 dark:bg-gray-900/30">
            ...
            {metaPolicies.length === 0 ? (
              <div className="p-6 border-2 border-dashed border-border rounded-lg bg-muted/10 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('syllabus.policies.noPolicies', 'No policies added yet.')}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {metaPolicies.map((policy) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <div className={cn('p-1 rounded-md', clr)}>
                          <PIco className="h-3 w-3" />
                        </div>
                        {policy.type === 'custom' ? (
                          <Input
...
const SettingsField = ({
  ...
  return (
    <div>
      <span
        className={`text-xs font-bold text-muted-foreground uppercase tracking-wider block mb-0.5 ${isRTL ? 'text-right' : 'text-left'}`}
      >
        {label}
      </span>
      <p
