      ...
      {categories.length === 0 ? (
        ...
        <div className="space-y-2">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="flex items-center gap-3 p-3 rounded-xl border border-border bg-card shadow-sm group"
            >
              {editingId === cat.id ? (
                <>
                  <Input
                ...
                <>
                  <span
                    className={`flex-1 font-medium text-foreground text-sm ${isRTL ? 'text-right' : 'text-left'}`}
                  >
                    {cat.name}
                  </span>
          ...
          <div className={`flex items-center gap-2 px-3 pt-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            {totalWeight !== 100 && totalWeight > 0 && (
              <span className="text-xs text-muted-foreground">
                {t('syllabus.grading.shouldEqual100')}
              </span>
            )}
          </div>
        </div>
