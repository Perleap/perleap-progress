...
import { BookOpen, GraduationCap, Scale, Layers } from 'lucide-react';
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="rounded-xl border-border shadow-sm">
          ...
          <CardContent className="space-y-3">
            ...
            {data.courseDuration && <ReviewField label={t('createClassroom.courseDuration')} value={data.courseDuration} isRTL={isRTL} />}
        ...
        {/* Syllabus Summary */}
        <Card className={cn('rounded-xl border-border shadow-sm', !data.includeSyllabus && 'opacity-60')}>
          <CardHeader className="pb-3">
            <CardTitle className={`flex items-center gap-2 text-base ${isRTL ? 'flex-row-reverse' : ''}`}>
              <GraduationCap className="h-4 w-4 text-primary" />
              {t('syllabus.wizard.syllabusLabel')}
              {!data.includeSyllabus && (
                <Badge variant="secondary" className="rounded-full text-xs ms-2">{t('syllabus.wizard.skipped')}</Badge>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {data.includeSyllabus ? (
              <>
                ...
                <ReviewField
                  ...
                  isRTL={isRTL}
                />
                <ReviewField label={t('syllabus.settings.status')} value="draft" isRTL={isRTL} />
              </>
            ) : (
              <p className={`text-sm text-muted-foreground ${isRTL ? 'text-right' : 'text-left'}`}>
                {t('syllabus.wizard.canAddLater')}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="rounded-xl border-border shadow-sm">
            ...
            <CardContent>
              ...
              {/* Policies summary */}
              {(data.gradingPolicyText || data.lateWorkPolicyText || data.attendancePolicyText || data.communicationPolicyText) && (
                <div className="mt-4 pt-4 border-t border-border">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {data.gradingPolicyText && <Badge variant="outline" className="rounded-full text-xs">{t('syllabus.wizard.gradingPolicy')}</Badge>}
                    {data.lateWorkPolicyText && <Badge variant="outline" className="rounded-full text-xs">{t('syllabus.wizard.lateWorkPolicy')}</Badge>}
                    {data.attendancePolicyText && <Badge variant="outline" className="rounded-full text-xs">{t('syllabus.wizard.attendancePolicy')}</Badge>}
                    {data.communicationPolicyText && <Badge variant="outline" className="rounded-full text-xs">{t('syllabus.wizard.communicationPolicy')}</Badge>}
                  </div>
                </div>
              )}
            </CardContent>
