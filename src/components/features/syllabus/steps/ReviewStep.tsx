    ...
    <div className="space-y-6">
      <div className={isRTL ? 'text-right' : 'text-left'}>
        <h3 className="text-xl font-bold text-foreground mb-1">
          {t('syllabus.wizard.reviewTitle')}
        </h3>
        <p className="text-muted-foreground text-sm">{t('syllabus.wizard.reviewDesc')}</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="rounded-xl border-border shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle
              className={`flex items-center gap-2 text-base ${isRTL ? 'flex-row-reverse' : ''}`}
            >
          ...
          <CardContent className="space-y-3">
            ...
            {filteredDomains.length > 0 && (
              <div>
                <span
                  className={`text-xs font-bold text-muted-foreground uppercase tracking-wider block mb-1 ${isRTL ? 'text-right' : 'text-left'}`}
                >
            ...
            {filteredOutcomes.length > 0 && (
              <ReviewField
                label={t('syllabus.review.learningOutcomes')}
                value={t('syllabus.review.defined', { count: filteredOutcomes.length })}
                isRTL={isRTL}
              />
            )}
            {filteredChallenges.length > 0 && (
              <ReviewField
                label={t('syllabus.review.keyChallenges')}
                value={t('syllabus.review.defined', { count: filteredChallenges.length })}
                isRTL={isRTL}
              />
            )}
          </CardContent>
        ...
        <Card className="rounded-xl border-border shadow-sm">
          ...
          <CardContent className="space-y-3">
            <ReviewField
              ...
              value={data.syllabusTitle || data.courseTitle || '—'}
              isRTL={isRTL}
            />
            <ReviewField
      ...
      {data.includeSyllabus && (
        <p className={`text-xs text-muted-foreground ${isRTL ? 'text-right' : 'text-left'}`}>
          {t(
            'syllabus.wizard.reviewAssignmentHint',
            'After the classroom is created, link assignments to sections from Course Outline.'
          )}
        </p>
      )}
        ...
        <Card className="rounded-xl border-border shadow-sm">
          <CardHeader className="pb-3">
            ...
            >
              ...
              <Badge variant="secondary" className="rounded-full text-xs ms-2">
                {data.sections.length} {t('syllabus.tabs.sections')}
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span
                          className={`text-sm font-medium text-foreground block truncate ${isRTL ? 'text-right' : 'text-left'}`}
                        >
                        ...
                        {section.startDate && (
                          ...
                          >
                            {section.startDate}
                            {section.endDate ? ` → ${section.endDate}` : ''}
                          </span>
                        )}
                        <div className={`flex flex-wrap gap-1 mt-1 ${isRTL ? 'justify-end' : ''}`}>
                          {section.content?.replace(/<[^>]+>/g, '').trim().length ? (
                            <Badge
                              variant="outline"
                              className="rounded-full text-[10px] gap-0.5 h-5 px-1.5"
                            >
                          ...
                          {section.completion_status !== 'auto' ? (
                            <Badge
                              variant="outline"
                              className="rounded-full text-[10px] h-5 px-1.5"
                            >
          ...
          <CardHeader className="pb-3">
            ...
            >
              ...
              {filteredCategories.length > 0 && (
                <Badge variant="secondary" className="rounded-full text-xs ms-2">
                  {t('syllabus.review.categories', { count: filteredCategories.length })}
                </Badge>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {data.policies.length > 0 && (
              <div className="mt-4 pt-4 border-t border-border">
                <span
                  className={`text-xs font-bold text-muted-foreground uppercase tracking-wider block mb-2 ${isRTL ? 'text-right' : 'text-left'}`}
                >
                  {t('syllabus.wizard.policies')}
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {data.policies.map((policy) => (
                    <Badge key={policy.id} variant="outline" className="rounded-full text-xs">
                      {policy.label}
                    </Badge>
                  ))}
                </div>
