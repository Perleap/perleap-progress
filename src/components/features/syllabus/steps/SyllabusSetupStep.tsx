    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6 p-6 rounded-xl border border-border shadow-sm">
        <div className={`flex items-center gap-2 text-primary ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <h3 className={`font-bold text-heading ${isRTL ? 'text-right' : 'text-left'}`}>
            {t('syllabus.wizard.syllabusDetails')}
          </h3>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder={
              data.courseTitle ? `${data.courseTitle} Syllabus` : 'e.g. Biology 101 - Spring 2026'
            }
      ...
      <div className="space-y-6 p-6 rounded-xl border border-border shadow-sm">
        ...
        <p className={`text-sm text-muted-foreground ${isRTL ? 'text-right' : 'text-left'}`}>
          {t(
            'syllabus.policies.addPolicyDesc',
            'Add only the policies relevant to your course. You can always add more later.'
          )}
        </p>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
                      {isCustom && (
                        <div className="space-y-1.5">
                          <Label className="text-xs font-medium">
                            {t('syllabus.policies.policyName', 'Policy Name')}
                          </Label>
                          <Input
                            value={policy.label}
                      ...
                      <Textarea
                        ...
                        placeholder={t(
                          'syllabus.policies.contentPlaceholder',
                              ...
                              {data.gradingCategories.map((cat) => (
                                ...
                                >
                                  <Input
                                    value={cat.name}
                                    onChange={(e) =>
                                      updateGradingCategory(cat.tempId, { name: e.target.value })
                                    }
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <Input
                                      ...
                                      value={cat.weight || ''}
                                      onChange={(e) =>
                                        updateGradingCategory(cat.tempId, {
                                  ...
                                  <Button
                                    ...
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => removeGradingCategory(cat.tempId)}
        ...
        {!hasGradingPolicy && data.gradingCategories.length > 0 && (
          <div className="p-3 rounded-lg border border-orange-200 bg-orange-50 dark:border-orange-900/30 dark:bg-orange-900/10">
            <p className="text-xs text-orange-700 dark:text-orange-400">
              {t(
                'syllabus.policies.gradingCategoriesOrphan',
                'You have grading categories defined but no Grading Policy. Add a Grading Policy to manage them, or they will be saved independently.'
              )}
            </p>
          </div>
        )}
