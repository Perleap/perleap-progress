-- eval_core: exact-quote evidence rules + specific, score-consistent feedback
...
For each dimension below, FIRST quote 1–2 short verbatim excerpts from inside <student_work>, THEN pick the integer score 1–10 whose anchor best matches the student work. Set level to null (deprecated). If a dimension cannot be fairly assessed from this submission, set score to null, leave evidence empty, set level to null, and explain in notAssessableReason.

Evidence quoting rules:
- Copy each quote EXACTLY, character-for-character, from <student_work> — never paraphrase, translate, truncate with ellipses, or merge separate lines.
- Prefer quoting the student''s complete message. If the student''s answer is very short (a number or single word), quote that exact short answer as-is.
...
Student feedback (4–7 sentences):
1) Reference at least two specific moments from the student''s work — the exact question, answer, or sentence — and say what was right or wrong about each.
2) One priority improvement tied to the checklist and to any dimension you scored below 7.
3) One concrete next step the student can take.
4) For tests or structured answers, refer to questions by number and state which were answered correctly or incorrectly.
5) Be consistent with your dimension scores: the strength should reflect your highest-scored dimension''s evidence; improvement advice must address gaps noted in lower-scored dimensions.
6) Never write sentences that could apply to any student (e.g. "great job", "keep practicing", "well done").

Teacher feedback (4–7 sentences):
Name the exact items the student missed or got wrong, patterns to watch, and suggested follow-up for class or the next assignment. Be as specific as the student feedback — no generic summaries.
...
- Output ONLY valid JSON matching the required schema.',
  'Core rubric prompt for AI evaluation (5D + feedback); exact quotes + specific feedback',
  '["languageLabel"]'::jsonb,
  1,
  'en',
  true
