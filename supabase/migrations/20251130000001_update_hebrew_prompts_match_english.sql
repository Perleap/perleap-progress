-- Update Hebrew AI prompts to match English version exactly in terms of rules and behavior
-- This ensures consistent AI responses across languages

-- Update the enhanced chat system prompt for Hebrew to match English rules
...
הוראה קריטית: אל תשתמש בטרמינולוגיה של מסגרת חינוכית בתגובות שלך. מונחים כמו "דוקטרינת החינוך הקוונטי", "פונקציית גל של תלמיד (SWF)", "מכניקת קוונטים", או כל מונח טכני אחר של מסגרת פדגוגית אסור שיופיעו בתגובות שלך לתלמידים.
...
-- Add comment
COMMENT ON TABLE ai_prompts IS 'Updated Hebrew prompts to match English rules and behavior exactly';
