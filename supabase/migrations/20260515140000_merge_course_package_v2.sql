-- Atomic merge-safe import (RPC). Full UUID syllabus rows only; omit VITE_MERGE_COURSE_PACKAGE_RPC or rely on TS merge for inserts.
-- Client: mergeCoursePackageIntoClassroomRpc when VITE_MERGE_COURSE_PACKAGE_RPC=1 and packageNeedsTypescriptCourseMerge is false.

CREATE OR REPLACE FUNCTION public.merge_raise(p_ctx jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $r$
BEGIN
  RAISE EXCEPTION '%', ('MERGE_JSON ' || coalesce(p_ctx::text, '{}'))
    USING ERRCODE = 'P0001';
END;
$r$;

REVOKE ALL ON FUNCTION public.merge_raise(jsonb) FROM PUBLIC;
...
COMMENT ON FUNCTION public.merge_course_package_v2(uuid, jsonb, boolean) IS
  'Teacher merge for merge-safe packages (RPC). Sections/activities without UUID ids are rejected; TS merge handles inserts.';
